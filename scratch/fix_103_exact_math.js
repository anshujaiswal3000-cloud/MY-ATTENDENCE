...
async function fix103Math() {
  try {
    ...
    const historyLogs = []
    const autoLoggedSlots = []
    const subjectCounts = {}

    // Date range: 13/07/2026 to 28/07/2026 (up to 03:40 PM today)
    const startDate = new Date('2026-07-13T09:00:00')
    const endDate = new Date('2026-07-28T15:40:00')
    ...
    while (cur <= endDate) {
      ...
      if (!isSunday && !isFirstSaturday && !isThirdSaturday) {
        ...
        daySlots.forEach(slot => {
          ...
          // Today (28/07/2026): Include slots up to P8 (03:40 PM) -> CO, DLD, DS, DS_LAB, DSTL, PSD
          if (dFormatted === '28/07/2026') {
            if (['09:00 AM - 09:50 AM', '09:50 AM - 10:40 AM', '10:40 AM - 11:30 AM', '11:30 AM - 01:10 PM', '02:00 PM - 02:50 PM', '02:50 PM - 03:40 PM'].includes(slot.time)) {
              ...
              const slotKey = `${dFormatted}_${exactSubjectId}_${dayName}_${slot.time}`
              autoLoggedSlots.push(slotKey)
    ...
    console.log('\n========================================')
    console.log('📊 ACCURATE NON-DUPLICATED ATTENDANCE MATH')
    console.log('========================================')
    let academicSum = 0
    Object.keys(subjectCounts).forEach(code => {
      const s = subjectCounts[code]
      if (code !== 'LIBRARY-2') academicSum += s.present
      console.log(`${code} [${s.id}] | ${s.name}: ${s.present}/${s.total}`)
    })
    console.log(`\n🏆 Academic Total Sum: ${academicSum} (NOT 103!)`)
    console.log(`Total History Logs: ${historyLogs.length}`)
    console.log(`Total AutoLoggedSlots Keys: ${autoLoggedSlots.length}`)
    console.log('========================================\n')

    // Build official subject array for DB
    const existingSubjects = userDoc.subjects
    ...
    console.log('✅ Baseline Cleaned Up & MongoDB Atlas Cloud Updated to Exact Math!')
    process.exit(0)
  } catch (err) {
