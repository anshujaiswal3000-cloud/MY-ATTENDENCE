...
async function syncDualTimetableHistory() {
  try {
        ...
        daySlots.forEach(slot => {
          ...
          if (dFormatted === '28/07/2026') {
            if (['09:00 AM - 09:50 AM', '09:50 AM - 10:40 AM', '10:40 AM - 11:30 AM', '11:30 AM - 01:10 PM', '02:00 PM - 02:50 PM'].includes(slot.time)) {
              if (!subjectCounts[slot.code]) subjectCounts[slot.code] = { present: 0, total: 0, name: slot.name }
              subjectCounts[slot.code].present += slot.inc
              subjectCounts[slot.code].total += slot.inc

              historyLogs.push({
                id: `hist_${Math.random().toString(36).slice(2, 9)}`,
                subjectId: `subj_${slot.code.toLowerCase().replace(/[^a-z0-9]/g, '')}`,
    ...
    console.log('\n========================================')
    console.log('📊 ACCURATE DUAL-WEEK ATTENDANCE SUMMARY')
    console.log('========================================')
    Object.keys(subjectCounts).forEach(code => {
      const s = subjectCounts[code]
      console.log(`${code} | ${s.name}: ${s.present}/${s.total}`)
    })
    ...
    console.log('✅ Dual Timetable (Week 1 W.E.F 13/07 & Week 2/3 W.E.F 26/07) Data Successfully Synced to MongoDB Atlas!')
    process.exit(0)
  } catch (err) {
