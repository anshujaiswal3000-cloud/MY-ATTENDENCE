...
import {
  ...
  MenuItem, Chip, IconButton, InputAdornment
} from '@mui/material'
import { MdAdd, MdSearch, MdDelete, MdEvent, MdFolderSpecial, MdBookmark } from 'react-icons/md'
...
export default function Notes() {
  const { subjects, notes, addNote, deleteNote } = useAttendance()
    ...
    <Box>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3, flexWrap: 'wrap', gap: 2 }}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">Keep track of assignments, exam topics, and lecture notes</Typography>
        </Box>
        <Button
          variant="contained"
          startIcon={<MdAdd />}
      ...
      {filteredNotes.length === 0 ? (
        <EmptyState
          ...
          subtitle={search ? 'No notes match your search term.' : 'Click "Add Note" to create your first note or task!'}
        />
      ) : (
        <Grid container spacing={2.5}>
          {filteredNotes.map((note, i) => (
            <Grid item xs={12} sm={6} md={4} key={note.id || i}>
              <GlassCard delay={i * 0.05} sx={{ p: 2.5, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                <Box>
                  ...
                  <Typography variant="subtitle1" sx={{ fontWeight: 800, mb: .5, lineHeight: 1.3 }}>
                    {note.title}
                  </Typography>
                  <Typography variant="caption" sx={{ color: '#60a5fa', fontWeight: 700, display: 'block', mb: 1.5 }}>
                    📚 {note.subjectName}
                  </Typography>
                  <Typography variant="body2" sx={{ opacity: 0.85, whiteSpace: 'pre-wrap', lineHeight: 1.6 }}>
                    {note.content}
                  </Typography>
                </Box>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: .8, mt: 2, pt: 1.5, borderTop: '1px solid rgba(148,163,184,.12)' }}>
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: '20px', p: 1 } }}>
        <DialogTitle sx={{ fontWeight: 800 }}>Add New Subject Note</DialogTitle>
        <DialogContent>
          <Box component="form" onSubmit={handleCreate} sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
            ...
            <TextField
              ...
              placeholder="Enter lecture summary, assignment instructions, or important formulas..."
            />
          </Box>
        </DialogContent>
